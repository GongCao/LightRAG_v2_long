...
async def req_emb(query):
    ...
    data = {
        ...
        "token":"f0b7c68e-8eba-4c12-9d75-81cfd096ef3a",
        "stream":False,
        "bizRequestId":"test" + str(current_time),
        "chatContent":{
            "contentType":"text",
            "text":query
        },
    ...
    url = "https://ibotservice-pre.alipayplus.com/almpapi/v1/message/chat" # 预发环境
    ...
    result = json.loads(data['messageList'][0]['content'][0]['text'])
...
async def aoneai_complete_if_cache(
    prompt,
    system_prompt,
    lastMessageIds=[],
    base_url=None,
    ...
    **kwargs,
    ...
    # 定义接口的 URL
    url = base_url + '/v1/chat'  # 替换为您的实际 API URL

    print(lastMessageIds)
    chatMessage = []
    if system_prompt:
        chatMessage.append({"role": "system", "content": system_prompt})
    chatMessage.append({"role": "user", "content": prompt})

    # 构造请求体
    payload = {
        "chatMessage": chatMessage,
        "stream": stream,  # 可选，默认值为 True
        "needAppend": True,  # 可选，默认值为 True
        "lastMessageIds": lastMessageIds  # 可选，替换为实际的消息 ID 列表
    }
...
async def whale_embedding(
    ...
    # 线上示例：TextGeneration.set_api_key("your-api-key")
